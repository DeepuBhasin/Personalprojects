...
if(isset($_POST['submit'])){
    ...
    $from_user_id = $_POST['send_to'];
    $to_user_id = $_POST['send_by'];
    ...
    $sql = "UPDATE offer set message_status=0 where id=$offer_id";
    
    $y = mysqli_query($con,$sql);

    if($x&&$y)
    {
        header('location:receiveInbox.php?id='.$offer_id.'&status=success&message='.urlencode('Message Sent Successfully'));
    }
    else
    {
        header('location:receiveInbox.php?id='.$offer_id.'&status=danger&message='.urlencode('Database Problem'));    
    }
...
if(isset($_GET['accepted'])){
   ...
   $accepted = $_GET['accepted']; 
    ...
    $sql = "UPDATE offer set accepted=$accepted where id=$offer_id";
    ...
    if($y)
    {
        if($accepted==2){
            ...
            $message = " Offe Rejected Successfully";
        }
        header('location:receiveInbox.php?id='.$offer_id.'&status=success&message='.urlencode($message));
    }
    else
    {
        header('location:receiveInbox.php?id='.$offer_id.'&status=danger&message='.urlencode('Database Problem'));    
                                ...
                                <div class="col-3">
                                    <div class="woocommerce-shipping-fields">
                                           <div class="shipping_address" style="display: block;">
                                                ...
                                                ?>    
                                                <a href="receiveInbox.php?id=<?= $offer_id?>&accepted=2" class="btn btn-success">Accept</a> <a href="receiveInbox.php?id=<?= $offer_id?>&accepted=3" class="btn btn-danger">Reject</a></td>
                                                <?php }?>
                                                </tr>
                                               </table> 
                                           </div>
