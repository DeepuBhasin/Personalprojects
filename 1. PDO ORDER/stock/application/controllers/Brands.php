	...
	*/ 
	public function fetchBrandData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '<a href="'.base_url("brands/action/").$value['id'].'" class="btn btn-sm btn-success">View Profile</a>';

			/*$buttons = '<a href="products.php"><button type="button" class="btn btn-default" onclick="editBrand('.$value['id'].')" data-toggle="modal" data-target="#editBrandModal"><i class="fa fa-pencil"></i></button>'; */
	...
	*/
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	$data = array(
        		'code' => $this->input->post('code'), //name
        		'name' => $this->input->post('name'),
        		'category' => $this->input->post('category'),	//active
        	);
	...
	*/
	...
	public function action($id=null)
	{
		if (isset($_POST['actionButton'])) {
			$this->input->post();
			$this->model_brands->makeAction();
			exit;
		}
		
		$result = $this->model_brands->getSingleBrandData($id);
		$this->data['results'] = $result;
		$this->render_template('brands/action', $this->data);
	}
}