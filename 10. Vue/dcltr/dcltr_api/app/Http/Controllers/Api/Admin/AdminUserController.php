...
class AdminUserController extends Controller
{
    ...
    public function getUserData(Request $request, $user){
        ...
        }, 'types.company.country','types.company.operationAreas', 'types.company.state', 'types.company.city', 'types.company.references', 'types.company.categories','types.company.categories.category','types.company.categories.subcategory','addresses' ])->first();
        return response()->json($user);
    }

    public function getAllUsersByType(Request $request, $type){
        $perPage = $request->has('per_page')?$request->get('per_page'):10;
        return UserResource::collection(User::query()->{$type}()->orderBy('id', 'desc')->paginate($perPage));
    }
