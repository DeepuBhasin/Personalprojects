...
class CategoryController extends Controller
{
    public function getScrapCategoryList()
    {
        ...
        ->whereJsonContains('category_type', $categoryType)
        ...
        if($allSubCat){
            foreach ($allSubCat as $key => $cat) {
                ...
                if(!count($toKeepArr)){
                    ...
                    $cat->update(['category_type'=>array_values($toKeepArr)]);
                }
    ...
    public function getCategoryDetails(Request $request)
    {
        ...
        if (empty($categoryExist)) {
            ...
            $categoryDetail = Category::select(['id', 'title', 'parent_id', 'category_type', 'image_path'])
            ->where('id', $id)->first();
    ...
    public function createCategory(CreateCategoryRequest $request)
    {
        if(!empty($request->parent_id)) {
            ...
            //$ititle = $request->image->getClientOriginalName();
            $path = 'catPhotos/'.$imageName;
            $data = Category::create($request->validated() + ['image_path'=>$path, 'category_type' => $categoryType->getRawOriginal('category_type')]);
            // $validated = $request->validated();
    ...
    {
        $categories = Category::whereNull('parent_id')->with('form')->orderBy('id', 'DESC')->paginate();
        return new CategoryCollection($categories);
    }
