...
class ResponseController extends Controller
{
    ...
    public function getProductRequestResponses()
    {
        ...
        ->whereHas('products', function($q){
            $q->where('sale_option_id', '!=', config('constants.sellOptions.toss'));        
        })->latest()->get()->toArray();
        $bids = ProductBidding::where('added_by', $user->id)->where('status', '!=', config('constants.bidStatus.assign_by_admin'))
            ->with('products')->latest()->get()->toArray();
        $responses = array_merge($responses, $bids);
        foreach ($responses as $datakey => $response) {
            foreach (config('constants.responseStatus') as $status => $val) {
                if (isset($responses[$datakey]['status']) && $responses[$datakey]['status'] == $val) {
