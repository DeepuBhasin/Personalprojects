...
class ServiceRequestController extends Controller
{
    ...
    {
        $user = auth()->user();
        $products = Product::select(['id', 'title', 'description', 'price', 'user_id', 'sale_option_id', 'status', 'is_bid_accepted'])
        ...
        foreach ($allServiceRequest as $serviceRequest) {
            ...
            if (empty($response) || $response->created_at < Carbon::now()->subHour(1)) {
                ...
                $serviceProviders = DB::table('user_companies')
                    ...
                    ->where('user_companies.user_type_id', config('constants.userRoles.Service Provider'))
    ...
    public function deleteService(ServiceRequest $service): JsonResponse
    {
        $service->delete();
        return response()->json([
            'message'=>__('apiMessage.serviceDeleted'),
            'status'=> 'success'
        ]);
    }
    ...
    {
        $services = ServiceRequest::whereIn('id', Response::where('user_id', auth()->id())
            ->where('type', 'service')->pluck('request_id'))
            ->with('user')
        ->get();
        return ServiceRequestResource::collection($services);
    }
