<template>
    <SectionHeader header="Transactions" :titleBoolen="true">
        ...
        <template v-slot:title>
            <h3 class="card-title">Transactions Management</h3>
        ...
        <template v-slot:body>
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <ITable :classes="classes" :perPageLimit="true" :columns="columns" :rows="rows" :meta="meta" section="transactions"></ITable>
                </div>
            </div>
        </template>
    </SectionHeader>
...
export default {
    ...
    watch: {
        ...
        '$route.params.id': {
            handler: function(id) {
                api.get(`admin/transactions/${id}`).then(data=>data.data).then(response=>{ 
                    console.log("response",response); 
                    let rows = response.data.map(function(obj){
                        console.log("Am obj", obj);
                        switch (obj.payment_type) {
                            case 1:
                                obj['payment_type'] = "Bid Payment To Admin"
                                break;
                            case 2:
                                obj['payment_type'] = "Bank Payment To Seller"
                                break;
    ...
    data(){
        return {
            ...
            columns: [
                ...
                {'title':'Product', key:'order.product.title'},
                {'title':'Seller', key:'email'},
                {'title':'Bidder', key:'roles'},
