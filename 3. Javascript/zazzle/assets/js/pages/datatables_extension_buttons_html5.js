...
$(function() {
    ...
    var dtable = $('#example').DataTable({
             "ajax": "https://raw.githubusercontent.com/zarizahra/test/master/MOCK_DATA.json",
             "scrollCollapse": true,
        "columns": [
            { 
                "className":      'truncate',
            ...
            render: function (data, type, row, meta) {
                ...
                for (var i = 0; i < data.length; i++) {
                    // console.log(data[i].name);
                     newdata += '<span class="label label-danger">'+data[i].name+'</span>  ' ;
                }
        ...
        columnDefs: [
                   ...
                   //{ targets: [4], className: 'label label-danger'},
        ],
        ...
        //console.log(this.api().columns());
            this.api().columns().every(function(i) {
                ...
                var select = $('<select class="select2 sel2Filter" data-placeholder="Filter"><option value=""></option></select>')
                ...
                column.data().unique().sort().each(function (d, j) {
                    if(/<[a-z][\s\S]*>/i.test(d))
                    {
                        if($(d).find('span'))
                        {
                            d = $(d).find('span').replaceWith(function(){ return this.innerHTML;}).end();
                            d = d[0].innerText;
                        }
                        else
                        {
                            d = $(d).find('a').replaceWith(function(){ return this.innerHTML;}).end();
                            d = d[0].text;     
                        }
...
// Visibility Changed
    $('#example').on( 'column-visibility.dt', function ( e, settings, column, state ) {
            ...
            Dtable.api().columns().every(function(i) {
                var column = this;
                //console.log(column.data());
                var select = $('<select class="select2 sel2Filter" data-placeholder="Filter"><option value=""></option></select>')
                    .appendTo($(column.footer()).not(':last-child').empty())
    ...
    $(document).ready(function(){
                        ...
                        if ( row.child.isShown() ) {
                            ...
                            // Open this row
                            row.child( format(row.data()), 'child-shown' ).show();
                            tr.addClass('shown');
                                                    }
                    } );
                });
...
// Child rows formatting function
  function format ( d ) {
    // console.log(d);
    // `d` is the original data object for the row
    return '<table width=100% cellpadding="10">'+
        '<tr>'+
            '<td width="20%" colspan="8" style="color: #5C5B5C;	font-size: 16px">'+d.term+'</td>'+
        '</tr>'+
        '<tr>'+
            '<td width="50%" colspan="4" rowspan="1" class="td-child-label">Description</td>'+
            '<td colspan="1" class="td-child-label">First Deployed</td>'+
            '<td colspan="1">'+d.firstdeployed+'</td>'+
            '<td colspan="1" class="td-child-label">Data Owner</td>'+
            '<td colspan="1">'+d.dataownerfullname+'</td>'+
        '</tr>'+
        '<tr>'+
            '<td width="50%" colspan="4" rowspan="2" style="vertical-align:top;">'+d.description+'</td>'+
            '<td colspan="1" class="td-child-label">Last Deployed</td>'+
            '<td colspan="1">'+d.lastdeployed+'</td>'+
            '<td colspan="1" class="td-child-label">Required?</td>'+
            '<td colspan="1">'+d.required+'</td>'+
        '</tr>'+
        '<tr>'+
            '<td colspan="1" class="td-child-label">Format</td>'+
            '<td colspan="1">'+d.format+'</td>'+
            '<td colspan="1" class="td-child-label">SFDC API Name</td>'+
            '<td colspan="1">'+d.sfdcapiname+'</td>'+ 
        '</tr>'+
        '<tr>'+
            '<td width="50%" colspan="4" rowspan="1" class="td-child-label">Business Rule</td>'+
            '<td colspan="1" class="td-child-label">Object</td>'+
            '<td colspan="1">'+d.object+'</td>'+
            '<td colspan="1" class="td-child-label">Tags</td>'+
            '<td colspan="1"><span class="label label-danger">'+d['tags'][0]['name']+'</span>'+'&nbsp;'+'<span class="label label-danger">'+d['tags'][1]['name']+'</span><span></td>'+
        '</tr>'+
        '<tr>'+
            '<td width="50%" colspan="4" rowspan="1" style="vertical-align:top; padding-right:10px">'+d.businessrule+'</td>'+
            '<td colspan="1" class="td-child-label">Source</td>'+
            '<td colspan="1">'+d.datasource+'</td>'+
        '</tr>'+
    '</table>';  
}
$(document).ready(function(){
